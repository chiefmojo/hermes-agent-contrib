        ...
        try:
            while True:
                ...
                if should_edit and self._accumulated:
                    ...
                    # Segment break or stream end: finalize the current message
                    # so platforms that need explicit closure (e.g. DingTalk AI
                    # Cards) don't leave the message in a loading state.  On
                    # got_done we pass finalize=True here so any per-adapter
                    # post-processing (e.g. Discord table conversion) fires in
                    # this single edit rather than requiring a redundant second
                    # call.
                    current_update_visible = await self._send_or_edit(
                        display_text,
                        finalize=got_segment_break or got_done,
                    )
                    self._last_edit_time = time.monotonic()
                ...
                if got_done:
                    ...
                    if self._accumulated:
                        if self._fallback_final_send:
                            ...
                            # The pre-done edit above already delivered the
                            # final content with finalize=True.  No further
                            # edit needed.
